...
*/
	...
	// configure options:
	$urlbase = "https://p.l/"; // <--- CONFIGURE ME! configurable - url address of the pastebin
	...
	$data_len_max = 256*1024; // configurable - maximum size of text (in bytes) at one past
	$data_len_maxfile = 4*1024*1024; // configurable - maximum size of file upload (in bytes) at one paste
	$totall_limit = 100 * 1024 * 1024; // bytes - maximum size of sum of ALL pastes as counted in the counter file
	...
	$metadata_include_sha = 1;  // in the -info metadata file should we append checksum of the entire text? for text only.
	...
	*/
	...
	// change if you updated this script:
...
<?php
	...
	if ($data_real && $data_ok) { // we will do the paste probably
		...
		if ($counter[0]===false) { $data_ok=0; // some counter error, probably limit
			...
			if (isset($_POST['prename'])) {
				$prename = urlencode( $_POST['prename'] ); // TODO better sanitize!
				if (strlen($prename) > 0) {
			...
			$file_ext = 'txt';
			if (! $data_is_text) {
				$file_ext = pathinfo($data_file_name, PATHINFO_EXTENSION);
			} 
			$filename1 = $filename.'.'.$file_ext;
			$filename2 = $filename.'.ini'; // for metadata
			...
			if (! @file_put_contents($filename2, $info)) die('Error writting the file (info)');
			echo '<p>Pasted: <b><a href="'._txt($url1).'">'._txt($url1).'</a></b> '
				.' [+<a href="'._txt($url2).'">info</a>]'
				.'</p>';
			$msg_yet=true;
		} // the paste.
	} // probably will paste
	...
	if (!$msg_yet) { // no message (error or the success url) written yet? then write this prompt:
		echo '<p>Paste the text in box below, it will be <b>published</b> on this page on generated shortened URL.</p>';
	}
?>
...
<div>
	<form method="POST" enctype="multipart/form-data" >
		...
		- upload file, or enter text:<br/>
		<input type="text" name="prename" value="" /> prepend name?<br/>
		<textarea name="text" cols="80" rows="30"><?php
...
<hr/>
<a href="<?php echo _txt($author_homepage);?>">Trivial Paste</a> 0.93 by <a href="https://github.com/rfree">rfree</a>.
